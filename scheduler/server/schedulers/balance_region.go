...
func (s *balanceRegionScheduler) Schedule(cluster opt.Cluster) *operator.Operator {
	...
	for _, store := range cluster.GetStores() {
		// a suitable store should be up && the down time cannot be longer than MaxStoreDownTime of the cluster
	...
	// destination is the store which not contain the chosen peer
	for j := 0; j < i; j++ {
		if _, ok := storeIds[stores[j].GetID()]; !ok /*not contain*/ {
			dstStore = stores[j]
			break
		}
	}
	...
	// So we have to make sure that the difference has to be bigger than two times the approximate size of the region,
